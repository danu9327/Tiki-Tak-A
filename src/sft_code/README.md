...
## 심리상담데이터(risk), sns대화데이터(sft)를 정제 및 변환하고
## EXAONE-3.5-7.8B를 LoRA로 파인튜닝하여 "또래 페르소나" 챗봇을 만들 코드들
...
단계	입력	출력
0_SNSdata_select	data/jsons/ (SNS 원본)	sft_from_sns_data.jsonl
1_filter_counseling	sft_from_risk_data.jsonl (362,107건)	sft_counseling_filtered.jsonl (30,000건)
2_convert_tone	sft_counseling_filtered.jsonl	sft_counseling_peer_tone.jsonl
3_merge_data	상담 + SNS jsonl	sft_total.jsonl
4_train_exaone	sft_total.jsonl	models/tuned/exaone_sft_lora/
...
### 32077MiB /  32607MiB <- 거의 터지기 직전ㅋㅋㅋ
### 하이퍼파라미터
