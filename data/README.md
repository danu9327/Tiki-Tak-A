...
### src/location_code/1_fetch_api.py로 9개의 위치 데이터(json) 수집
...
### data/rag/vector_db/는
### ChromaDB로 구축된 Vector DB로 미리 스포하자면 `src/rag_code/3_build_chroma.py` 실행해서 만든거에요
### `main_app.py` 실행 시 자동으로 연결될 진짜 DB임.
